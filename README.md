...
- **실제값 기준 연쇄 재예측:** 실제 가격 파일만 추가해도 기존 예측 월을 년월 기준으로 실제값으로 교체하고, 사이에 비어 있는 월은 이전 예측 스냅샷으로 연결합니다. 이후 월은 새 실제값을 전월 가격으로 사용해 다시 연쇄 예측하며, 화면에는 실제값과 연결 예측 구간을 구분해 표시합니다.
